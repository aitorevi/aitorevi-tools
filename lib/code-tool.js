...
import { createWrapToggle } from "./wrap-toggle.js";
 ...
 * @param {string} [opts.sample]            ejemplo precargado en la entrada (opcional)
 * @param {string} [opts.inputLabel]        etiqueta accesible de la entrada (opcional)
 * @param {Object} [opts.select]            desplegable de opciones: { labelKey, choices:[{value,label}], value }.
 *                                          Su valor se pasa como 2º argumento a format/minify.
 * @param {boolean} [opts.wrapInput=false]  ajuste de línea inicial de la entrada
 * @param {boolean} [opts.wrapOutput=false] ajuste de línea inicial de la salida
 */
export function mountCodeTool({ mount, toolId, format, minify, sample, inputLabel, select, wrapInput = false, wrapOutput = false }) {
      ...
      <div class="code-io">
        <label class="code-pane">
          <div class="code-bar">
            ...
            <span class="code-bar-actions"></span>
          </div>
        ...
        <div class="code-pane code-pane-out">
          <div class="code-bar">
            <span class="code-dots" aria-hidden="true"><i></i><i></i><i></i></span>
            <span class="code-bar-label">${M.codeOutput}</span>
            <span class="code-bar-actions">
  ...
  // Botón de ajuste de línea (word-wrap) en la barra de cada panel.
  const wrapLabels = { wrap: M.codeWrap };
  input.closest(".code-pane").querySelector(".code-bar-actions").append(createWrapToggle(input, wrapInput, wrapLabels));
  output.closest(".code-pane-out").querySelector(".code-bar-actions").prepend(createWrapToggle(output, wrapOutput, wrapLabels));
  ...
  const run = async (fn) => {
    ...
    try {
      // `await` admite tanto funciones síncronas (JSON, XML…) como asíncronas
      // (quicktype-core en JSON→C#).
      setOutput(await fn(input.value, selectEl ? selectEl.value : undefined));
    } catch (e) {
      setOutput("");
  ...
  // Al cambiar de opción (p. ej. dialecto SQL), re-formatea si hay entrada.
  if (selectEl) selectEl.addEventListener("change", () => { if (input.value.trim()) run(format); });
  ...
  // Precarga el ejemplo en la entrada (la salida queda vacía hasta que el
  // usuario pulse "Formatear"); sincroniza el estado de los botones.
  if (hasSample) input.value = sample;
  syncInputState();
}
