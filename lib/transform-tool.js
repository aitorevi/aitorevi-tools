 ...
 * @param {boolean} [opts.wrapInput=true]   ajuste de línea inicial de la entrada
 * @param {boolean} [opts.wrapOutput=true]  ajuste de línea inicial de la salida
 */
export function mountTransformTool({ mount, toolId, transform, select, sample, inputLabel, outputLabel, wrapInput = true, wrapOutput = true }) {
  ...
  root.innerHTML = `
    <div class="code-tool code-tool--sm">
      ${controls}
      <div class="code-io">
  ...
  const outActions = output.closest(".code-pane-out").querySelector(".code-bar-actions");
  outActions.appendChild(copyBtn);
  const syncCopy = wireCopyButton(copyBtn, () => output.value, labels);

  // Botón de ajuste de línea en cada panel (por defecto activado en estas tools).
  ...
  outActions.prepend(createWrapToggle(output, wrapOutput, wrapLabels));
